...
async function createBaseTour(
  page: Page,
  {
    ...
    favoriteFirst = false,
    face = false,
  }: { dresses?: number; favoriteFirst?: boolean; face?: boolean } = {},
) {
  await page.goto("/");
...
test("mobile layouts do not horizontally overflow from 320px to 430px", async ({
  page,
}) => {
  for (const size of [
...
test("candidate filter never blocks choosing two dresses for comparison", async ({
  ...
  await expect(
    page.getByRole("button", { name: /선택한 2벌 비교하기/ }),
  ).toBeEnabled();
});
...
test("detail selection stops at four items without corrupting existing picks", async ({
  page,
}) => {
...
test("invalid import error stays visible instead of disappearing as a toast", async ({
  page,
}) => {
