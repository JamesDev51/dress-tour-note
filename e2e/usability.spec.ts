...
test("theme and font preferences persist, then reset with local data", async ({
  page,
}) => {
  ...
  await page.reload();
  await expect(page.getByRole("button", { name: /클린/ })).toHaveAttribute(
    "aria-pressed",
    "true",
  );
  ...
  await page.goto("/privacy");
  await expect(page.getByRole("button", { name: /크림/ })).toHaveAttribute(
    "aria-pressed",
    "true",
  );
  await expect(page.getByRole("button", { name: /고딕/ })).toHaveAttribute(
    "aria-pressed",
    "true",
  );
});
...
test("editor choices remain independent across the 320 and 390px boundaries", async ({
  ...
  await page.getByRole("button", { name: /브이형/ }).click();
  await expect(page.getByRole("button", { name: /브이형/ })).toHaveAttribute(
    "aria-pressed",
    "true",
  );
  for (const width of [320, 390]) {
...
test("bad compare URL recovers safely and recent tour deletion works", async ({
  ...
  await expect(
    page.getByText("비교할 드레스 2벌을 다시 선택해 주세요."),
  ).toBeVisible();
  await page.goto("/");
  page.once("dialog", (dialog) => dialog.accept());
