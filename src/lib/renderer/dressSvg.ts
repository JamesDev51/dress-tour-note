...
function pattern(dress: PresentedDress, id: string) {
  switch (dress.fabric) {
    case "lace":
...
export function dressSvgMarkup(
  ...
  const base = colorHex[presented.color];
  const shadow = "#d7cbc5";
  const pat = pattern(presented, id);
  ...
  const face =
    includeFace && faceDataUrl
      ...
      : "";
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 360 640" role="img" aria-label="${esc(presented.label)} 드레스 미리보기"><defs><clipPath id="face-${id}"><ellipse cx="180" cy="70" rx="43" ry="49"/></clipPath>${pat}</defs><rect width="360" height="640" fill="#fbf8f6"/><image href="${esc(personDataUrl)}" x="0" y="0" width="360" height="640" preserveAspectRatio="xMidYMid meet"/>${face}<path d="${necklinePath(presented.neckline)}" fill="${fill}" stroke="${shadow}" stroke-width="2"/>${upperExtras(presented.topStyle, base, shadow)}<path d="${skirtPath[presented.silhouette]}" fill="${fill}" stroke="${shadow}" stroke-width="2"/></svg>`;
}
