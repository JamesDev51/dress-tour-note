...
export async function inspectPortablePdf(file:File):Promise<ImportPreview>{
  ...
  const pdfjs=await import('pdfjs-dist/legacy/build/pdf.mjs');pdfjs.GlobalWorkerOptions.workerSrc=workerUrl;let doc;try{doc=await pdfjs.getDocument({data:new Uint8Array(await file.arrayBuffer()),isEvalSupported:false}).promise}catch{throw new Error('파일을 읽을 수 없어요. 원본 PDF를 다시 선택해 주세요.');}
