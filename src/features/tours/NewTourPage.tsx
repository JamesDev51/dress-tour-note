...
import { createTour } from "../../db/repositories";
    ...
    <main className="min-h-dvh px-5 pb-28 pt-[calc(18px+env(safe-area-inset-top))]">
      ...
      <div className="mt-9 space-y-5">
        <label className="block text-sm font-semibold">
          신부 이름 <span className="font-normal text-stone-300">선택</span>
          <input
            maxLength={30}
            value={brideName}
        ...
        <label className="block text-sm font-semibold">
          투어 제목 <span className="font-normal text-stone-300">선택</span>
          <input
            maxLength={50}
            value={title}
      ...
      <div className="fixed bottom-0 left-1/2 w-full max-w-[480px] -translate-x-1/2 border-t border-stone-100 bg-[#fffdfa]/95 px-5 pb-[calc(16px+env(safe-area-inset-bottom))] pt-3 backdrop-blur">
        <button
          ...
          onClick={async () => {
            ...
            setSave("saving");
            try {
              const id = await createTour({ brideName, tourDate: date, title });
              setSave("saved");
